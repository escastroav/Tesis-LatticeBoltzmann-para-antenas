...
vector3D Corner::Interpolation(int ix1, int ix2,int iy1,int iy2,double x,double y,bool getB)
{
  ...
  E0=f11*F11+f12*F12+f21*F21+f22*F22;
  return E0;
}
void Corner::RadiationPattern(int&t,double r)
{
  vector3D E0, B0, S0;
  double x=0,z=0,u=0,v=0;
...
void Corner::ImprimirCorner(const char* fileName,bool useNew)
{
  ...
  for(double ix=0;ix<Lx0;ix+=1.0)
    for(double iz=0;iz<Lz0;iz+=1.0)
      {
	D0=D(ix,iy0,iz,false);	E0=E(D0,Epsr);
	B0=B(ix,iy0,iz,false);	H0=H(B0,Mur);
	S0=S(E0,H0);
	outputFile
	<< ix*size << "\t"
	<< iz*size << "\t"
  ...
  for(ia=0;ia<n;ia++)
    {
      ...
      x = r2*cos(angle)+ix0;
      z = r2*sin(angle)+iz0;
      Eteorico = (cos(M_PI*0.5*cos(angle))/sin(angle))*(cos(M_PI*0.5*cos(angle))/sin(angle));
      outputFile
	<< x << "\t"
	<< z << "\t"
	<< angle << "\t"	      
...
int main(int argc, char * argv[])
{
  if(argc != 4)
    {
      cout << "Wrong parameters!! ./a.out periods Sfile patternfile";
      return -1;
      }
  ...
  double T0=corner.GetT();
  int t=0,NT=atoi(argv[1]),tmax=NT*T0,T3=(NT-1)*corner.GetT(),tp=0;
  double r0=NT-1;
