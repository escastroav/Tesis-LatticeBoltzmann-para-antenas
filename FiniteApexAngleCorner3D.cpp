...
private:
  ...
  double sigma0=10,apexAngle=M_PI/3.0,alpha=0.8,thickness=1,order=0.5,delta=1;
...
Corner::Corner(double order0)
{
  ...
  offset=2;
  width=1.5*lambda;
  length=1*lambda*cos(apexAngle);
  sigma0=lambda/(25*M_PI*mu0*C);
  order=0.3;

  ix0=order0*0.5+Lx0/2;
  iy0=Ly0/2;
  iz0=Lz0/2;
...
double Corner::SigmaReflectors(int ix, int iy, int iz)
{
  ...
  double py = 0.5*(1-tanh(alpha*(-iyp-ixp/tan(apexAngle))));
  double px = 0.5*(1+tanh(alpha*(-iyp+ixp/tan(apexAngle))));
  //en X=0 inf
  double qx = 0.5*(1-tanh(alpha*(-offset-iyp-ixp/tan(apexAngle))));
  double qy = 0.5*(1+tanh(alpha*(offset-iyp+ixp/tan(apexAngle))));
  //bothplates
  double p = 1-px*py;
  double q = qx*qy;
  //cut_W
  double cut_W = 0.25*(1+tanh(alpha*(length-iyp)))*(1+tanh(alpha*(length+iyp)));
  double cut_H = 0.25*(1+tanh(alpha*(width+izp)))*(1+tanh(alpha*(width-izp)));
  //real plates
  double plates = sigma0*cut_W*cut_H*p*q;
