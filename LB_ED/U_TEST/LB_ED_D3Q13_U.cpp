...
const double factor=Lz/200;

const double eps0=1.0;
const double mu0=1.0;//1.0/(C*C*eps0);
const double C=0.707016; // C<0.707 celdas/click
	...
	for(int p=0;p<3;p++)
	  for(int i=0;i<4;i++)
	      ...
	      for(int r=0;r<2;r++)
		{
		  ...
		  f0[jx][jy][iz][r] = f0new[ix][iy][iz][r];
		}
}
...
{
  double Eo=0.001,Bo=Eo/C,alp=0.01,T=C*t;
  int iz0=40,jz0=0;
  for(int ix=0;ix<Lx;ix++)
    for(int iy=0;iy<Ly;iy++)
      for(int iz=0;iz<Lz;iz++)
	for(int p=0;p<3;p++)
	  for(int i=0;i<4;i++)
	      ...
	      for(int r=0;r<2;r++)
		{
		  jz0=(iz0+(int)T+Lz)%Lz;
		   D0[1]=0;	D0[2]=0;	B0[0]=0;	B0[2]=0;
		  D0[0] = Eo*exp(-alp*(iz-jz0)*(iz-jz0));	
		  B0[1] = Bo*exp(-alp*(iz-jz0)*(iz-jz0));
