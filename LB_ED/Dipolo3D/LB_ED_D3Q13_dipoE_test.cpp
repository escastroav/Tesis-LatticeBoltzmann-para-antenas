...
{//Jo=39.35
  double Eo=1,Bo=Eo/C,lambda=8,omega=2*M_PI*C/lambda,k=2*M_PI/lambda;
  int ix0=Lx/2,iy0=Ly/2,iz0=Lz/2,l4=(int)(0.25*lambda),yW2=2,alp=1/(2*yW2);
  //for(int iz=iz0-offset;iz<iz0+offset;iz++){
  //D0[1]=0;	D0[2]=0;	B0[0]=0;	B0[2]=0;
  
  for(int iz=iz0-2;iz<ix0+3;iz++){
    for(int iy=iy0-2;iy<iy0+3;iy++){
  for(int ix=ix0-l4;ix<ix0+l4+1;ix++){
    Eo = cos(k*(ix-ix0))*exp(-0.25*((iy-iy0)*(iy-iy0)+(iz-iz0)*(iz-iz0)));
    D0[0] = Eo*sin(omega*t);//exp(-0.75*(iz-iz0)*(iz-iz0));
    B0=B(ix,iy,iz,false);
  rho0=rho(ix,iy,iz,false);
  E0=E(D0,ix,iy,iz,false);	H0=H(B0,ix,iy,iz,false);
  J0=J(E0,ix,iy,iz,false);	
