...
{//Jo=39.35
  double Eo=1,Bo=Eo/C,T=16,omega=2*M_PI/T;
  int ix0=Lx/2,iy0=Ly/2,iz0=Lz/2;
  //for(int iz=iz0-offset;iz<iz0+offset;iz++){
  //D0[1]=0;	D0[2]=0;	B0[0]=0;	B0[2]=0;
  
  for(int ix=ix0-2;ix<ix0+3;ix++){
    for(int iy=iy0-2;iy<iy0+3;iy++){
      for(int iz=iz0-2;iz<iz0+3;iz++){
    Eo = 10*exp(-0.75*((ix-ix0)*(ix-ix0)+(iy-iy0)*(iy-iy0)+(iz-iz0)*(iz-iz0)));
    D0[2] = Eo*sin(omega*t);//exp(-0.75*(iz-iz0)*(iz-iz0));
    B0=B(ix,iy,iz,false);
  rho0=rho(ix,iy,iz,false);
  E0=E(D0,ix,iy,iz,false);	H0=H(B0,ix,iy,iz,false);
  J0=J(E0,ix,iy,iz,false);	
